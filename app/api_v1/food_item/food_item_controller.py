...
from ..database import food_item_collection
...
async def add_food_item_data(token: OAuthTokenDeps, food_item: CreateFoodItemDto = Body(...)):
    """
...
async def get_food_item_data(token: OAuthTokenDeps, id: str):
...
async def get_food_items(token: OAuthTokenDeps, params: ListQueryParams = Depends()):
...
@router.put("/{id}")
async def update_food_item_data(token: OAuthTokenDeps, id: str, req: UpdateFoodItemDto = Body(...)):
    """
...
async def delete_food_item_data(token: OAuthTokenDeps, id: str):
